...
<body>
    ...
    <div id="EditBtn" onclick="showE()" >
  	...
  	<div id="Edit" style="display: none">
  		...
  		<form enctype="multipart/form-data" method="post" action="<?php echo $_SERVER['PHP_SELF']; ?>">
  			<div id="EInput" id="">
            <label for="nur_id">工号：</label>
            <input type="text" id="nur_id" name="nur_id" /><br><br>
    ...
    <?php 
      ...
      if (isset($_POST['EditInfo'])) {
        ...
        $nur_id = mysqli_real_escape_string($dbc, trim($_POST['nur_id']));
        ...
        if (!empty($nur_id) && !empty($name) && !empty($age) && !empty($direction) && !empty($phone)) {
          $query = "SELECT * FROM nurse WHERE nur_id = '$nur_id'";
          $data = mysqli_query($dbc, $query);
          if (mysqli_num_rows($data) == 1) {
            // The nur_id is unique, so insert the data into the database
            // $query = "INSERT INTO nurse (nur_id, name, sex, age, direction, phone) VALUES ('$nur_id', '$name', '$sex', '$age', '$direction', '$phone' )";
            $query = "UPDATE nurse SET name = '$name', age = '$age', " .
              " direction = '$direction', phone = '$phone' WHERE nur_id = '$nur_id'";
      ...
      <div id="Dintro">
        ...
        <form enctype="multipart/form-data" method="post" action="<?php echo $_SERVER['PHP_SELF']; ?>">
          <div id="DInput">
            ...
            <input type="text" name="nur_id" id="nur_id">
        ...
        <?php 
          ...
          if (isset($_POST['DeleteInfo'])) {
            ...
            if (!$error) {
              if (!empty($nur_id)) {
                $query = "SELECT nur_id from nurse where nur_id = '$nur_id'";
                $data = mysqli_query($dbc, $query);
                if (mysqli_num_rows($data) == 1) {
                  $query1 = "DELETE FROM nurse WHERE nur_id = '$nur_id'";
                  mysqli_query($dbc, $query1);
                  
                  $home_url = 'http://' . $_SERVER['HTTP_HOST'] . dirname($_SERVER['PHP_SELF']) . '/manageNur.php';
                  //重定向到
                  echo '<script>window.location.href="'.$home_url.'";</script>';
      ...
      <div id="Add" style="display: none">
        <div id="Aintro">
          ...
          <form enctype="multipart/form-data" method="post" action="<?php echo $_SERVER['PHP_SELF']; ?>">
            <div id="AInput">
              <label for="ID">工号：</label>
              <input type="text" name="nur_id"><br><br>
              <label for="name">姓名：</label>
              <input type="text" id="name" name="name"/><br><br>
              <!-- <label for="sex">性别：</label>
              <input type="text" id="sex" name="sex"/><br><br> -->
          ...
          if (isset($_POST['AddInfo'])) {
            ...
            if (!empty($nur_id) && !empty($name) && !empty($age) && !empty($direction) && !empty($phone)) {
              ...
              if (mysqli_num_rows($data) == 0) {
                //The nur_id is unique, so insert the data into the database
                $query = "INSERT INTO nurse (nur_id, name, age, direction, phone) VALUES ('$nur_id', '$name', '$age', '$direction', '$phone' )";
                mysqli_query($dbc, $query);
                mysqli_close($dbc);
                $home_url = 'http://' . $_SERVER['HTTP_HOST'] . dirname($_SERVER['PHP_SELF']) . '/manageNur.php';
                //重定向到
  ...
  <script src="https://maxcdn.bootstrapcdn.com/bootstrap/3.3.7/css/bootstrap.min.css"></script>
	...
	<script src="js/myPaginationNur.js"></script>
